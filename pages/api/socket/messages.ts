...
import { SocketWithServer } from "@/types";
...
export default async function handler(
  ...
  try {
    ...
    if (!profile) return res.status(401).json({ message: "Unauthorised" });
    ...
    if (!member) return res.status(401).json({ message: "Unauthorised" });
    ...
    socket?.server?.io?.emit(channelKey, message);
